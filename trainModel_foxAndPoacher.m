...
%netA pools
pool('cur_con',179,'input');%some input units are not used; the model has only 71 active input units
pool('hidden1',100,'hidden'); 
pool('gestalt',100,'connection'); 
pool('prev_gestalt',100,'copyback',gestalt); 

%netB pools
pool('probe',182,'input');
pool('hidden2',100,'hidden'); 
pool('role_filler',182,'output');
...
generateRepresentations_foxAndPoacher; %generate feature based semantic representations for probe and output layer
...
for enum = 1:num_ep
    ...
    age = randi(100,1);
    pat = randi(100,1);
    generatePattern_foxAndPoacher; 
    
    e1 = file_environment('sentence_foxAndPoacher.pat');
    netA.environment = e1;
    e2 = file_environment('roleFiller_foxAndPoacher.pat');
    netB.environment = e2;
    ...
    if mod(enum,100000)==0
        weights_a = get_weights(netA);
        weights_b=get_weights(netB);
        fname = ['trainRes_foxAndPoacher_',num2str(model),'_', num2str(enum)];
        save (fname);
    end
    fclose('all');
end
...
filename = ['results_foxAndPoacher_',num2str(model),'_', num2str(enum)];
save(filename)