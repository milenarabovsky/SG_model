...
generateRepresentations_standard; %generate feature based semantic representations for the probe and output layer
...
for enum = 1:num_ep
    ...
    age = randi(100,1);
    pat = randi(100,1);
    generatePattern_standard; 
    
    e1 = file_environment('sentence_standard.pat');
    netA.environment = e1;
    e2 = file_environment('roleFiller_standard.pat');
    netB.environment = e2;    
    ...
    if (mod(enum,100000)==0)
        ...
        fname = ['trainRes_standard_',num2str(model),'_', num2str(enum)];
        save (fname);
    end
    fclose('all');
end

filename = ['results_standard_',num2str(model),'_', num2str(enum)];
save(filename)